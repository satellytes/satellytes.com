...
import { servicesTabsData } from '../../pages/services';

describe('Services Tabs', () => {
  it('should render', () => {
    const servicesTabs = render(<ServicesTabs tabs={servicesTabsData} />);
    expect(servicesTabs).toBeTruthy();
  });

  it('should show UX/UI tab', () => {
    const { getByText } = render(<ServicesTabs tabs={servicesTabsData} />);
  ...
  it('should focus Service Development tab heading when clicked', () => {
    const { getByRole } = render(<ServicesTabs tabs={servicesTabsData} />);
